...
export default function PrincipalDashboard({
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.user && (result.user.email === 'ynus335@gmail.com' || result.user.email === 'dkbmedia786@gmail.com')) {
      ...
      if (triggerNotification) {
        triggerNotification(
          "LOGIN ERROR ⚠️",
          error.message || "Pratigya network timeout ho gaya. Kripya bad me try karein.",
          "error"
        );
      }
    }
  ...
  const handleCredentialsLogin = (e: React.FormEvent) => {
    e.preventDefault();
    if (username.trim() === 'admin' && password === 'principal123') {
      setIsLoggedIn(true);
      setLoginError('');
      if (triggerNotification) {
        triggerNotification(
          "TERMINAL AUTH_OK 🔒",
          `Safar-e-Noorul Uloom login successful! Welcome back, Principal Saheb (Admin Offline).`,
          "success"
        );
      }
    } else {
      setLoginError('Incorrect Username or Password. Kripya sahi details darj karein.');
      if (triggerNotification) {
        triggerNotification(
          "ACCESS DENIED 🛑",
          "Aapka credentials ya login details galat hai.",
          "error"
        );
      }
    }
  ...
  const handleManualSheetLink = (e: React.FormEvent) => {
    e.preventDefault();
    if (!manualSheetId.trim()) return;
    
    // Parse Google Sheet URL or retrieve ID directly
    ...
    setSheetSyncMsg(`गूगल शीट आईडी मैन्युअल रूप से जोड़ दी गई है! Linked File ID: ${sheetId}. कृपया जांच लें कि आपकी गूगल शीट शेयर सेटिंग्स में "Anyone with the link can view (कोई भी लिंक वाला व्यक्ति देख सकता है)" पर सेट है, ताकि छात्र रिजल्ट देख सकें।`);
    setManualSheetId('');
    if (triggerNotification) {
      triggerNotification(
        "SHEET LINKED 📊",
        "Google Sheet manually connected successfully!",
        "success"
      );
    }
  };
  ...
  const handleExportLocalDatabase = () => {
    try {
      ...
      a.href = url;
      a.download = `noorul_uloom_db_backup_${new Date().toISOString().slice(0, 10)}.json`;
      ...
      if (triggerNotification) {
        triggerNotification(
          "BACKUP COMPLETE 💾",
          "Local database registry backup JSON file downloaded successfully!",
          "success"
        );
      }
    } catch (err: any) {
  ...
  const handleImportLocalDatabase = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!window.confirm("क्या आप सच में इस JSON बैकअप फ़ाइल से पूरे डेटाबेस को रीस्टोर करना चाहते हैं? इससे आपका मौजूदा लोकल डेटा रिप्लेस हो जायेगा।")) {
      e.target.value = '';
      return;
    }
    ...
    reader.onload = (event) => {
      try {
        ...
        if (triggerNotification) {
          triggerNotification(
            "RESTORE SUCCESSFUL 🔄",
            `Database restored cleanly across ${restoredCount} document domains.`,
            "success"
          );
        }
        alert("डेटाबेस सफलतापूर्वक रीस्टोर हो गया है!");
      } catch (err: any) {
  ...
  const handleCreateAndSyncSheet = async () => {
    ...
    try {
      ...
      let customMsg = error.message || 'Failed to trigger Google Sheets integration. Please try again.';
      if (error.code === 'auth/popup-closed-by-user' || String(error).includes('popup-closed-by-user')) {
        customMsg = '⚠️ त्रुटि: सत्यापन विंडो बंद कर दी गई थी। (Verification Popup was closed).\n\nयदि आपके जीमेल (ynus335@gmail.com) पर Google "Access Blocked" एरर आ रहा है, तो कृपया विंडो बंद कर दें और ऊपर दिए गए "Manually Link Google Sheet (गूगल शीट मैन्युअल लिंक करें)" सेक्शन का उपयोग करें। यह बिना किसी गूगल एरर के तुरंत काम करेगा!';
      } else if (error.code === 'auth/popup-blocked') {
        customMsg = '⚠️ ब्राउज़र ने पॉपअप विंडो ब्लॉक कर दी है! कृपया अपने ब्राउज़र में पॉपअप अनुमति दें (Allow Popups) और फिर से प्रयास करें, या सीधे मैन्युअल लिंक विकल्प का उपयोग करें।';
      }
      
      setSheetSyncMsg(customMsg);
    }
  };
  ...
  const handleManualSyncSheet = async () => {
    ...
    try {
      ...
      let customMsg = error.message || 'Failed to write record sets directly to Google Sheet.';
      if (error.code === 'auth/popup-closed-by-user' || String(error).includes('popup-closed-by-user')) {
        customMsg = '⚠️ त्रुटि: सत्यापन विंडो बंद कर दी गई थी। (Verification Popup was closed).\n\nयदि आपके जीमेल (ynus335@gmail.com) पर Google "Access Blocked" एरर आ रहा है, तो कृपया विंडो बंद कर दें और ऊपर दिए गए "Manually Link Google Sheet" सेक्शन का उपयोग करें। यह बिना किसी गूगल एरर के तुरंत काम करेगा!';
      } else if (error.code === 'auth/popup-blocked') {
        customMsg = '⚠️ ब्राउज़र ने पॉपअप विंडो ब्लॉक कर दी है! कृपया पॉपअप अनुमति दें या मैन्युअल गूगल शीट आईडी लिंक का उपयोग करें।';
      }
        ...
        <div className="bg-white dark:bg-slate-800 border-2 border-emerald-500/20 shadow-2xl p-8 rounded-3xl space-y-6 relative overflow-hidden">
          ...
          {loginMethod === 'google' ? (
            <form onSubmit={handleLogin} className="space-y-4">
              <button
                type="submit"
                className="w-full py-3 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 text-white rounded-xl font-bold text-sm shadow-md transition-transform active:scale-[0.98] flex justify-center items-center gap-2 cursor-pointer"
              >
                Sign in with Google (Admin Only)
              </button>
              
              {loginError && (
                <div className="space-y-2 p-3 bg-red-50 dark:bg-red-950/20 border border-red-200/50 dark:border-red-900/40 rounded-xl">
                  <p className="text-xs text-red-500 font-bold text-center uppercase leading-tight">{loginError}</p>
                </div>
              )}

              <div className="p-3 bg-blue-50/50 dark:bg-slate-900/40 border border-blue-100 dark:border-slate-800 rounded-xl text-[11px] text-slate-500 dark:text-slate-400 space-y-1 leading-normal">
                <p className="font-semibold text-blue-700 dark:text-emerald-400 flex items-center gap-1">
                  💡 Google Verification Blocked?
                </p>
                <p>
                  Agar Google authentication complete nahi hai ya testing constraints block bta rha hai, toh kripya upar <span className="font-bold text-emerald-700 dark:text-emerald-400">"Passcode Login"</span> tab select karein aur bypass credentials enter karein.
                </p>
              </div>
            </form>
          ) : (
            <form onSubmit={handleCredentialsLogin} className="space-y-4">
              <div className="space-y-1">
                <label className="block text-[11px] font-bold text-slate-600 dark:text-slate-400 uppercase tracking-wider">Username</label>
                <input
                  type="text"
                  required
                  value={username}
                  ...
                  className="w-full px-3.5 py-2 text-sm bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 text-slate-800 dark:text-white"
                />
              </div>

              <div className="space-y-1">
                <label className="block text-[11px] font-bold text-slate-600 dark:text-slate-400 uppercase tracking-wider">Passcode / Password</label>
                <input
                  type="password"
                  required
                  value={password}
                  onChange={(e) => setPassword(e.target.value)}
                  placeholder="e.g., principal123"
                  className="w-full px-3.5 py-2 text-sm bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 text-slate-800 dark:text-white"
                />
              </div>
              ...
              <button
                type="submit"
                className="w-full py-3 bg-gradient-to-r from-emerald-650 to-emerald-550 hover:from-emerald-700 hover:to-emerald-600 text-white rounded-xl font-bold text-sm shadow-md transition-transform active:scale-[0.98] flex justify-center items-center gap-2 cursor-pointer"
              >
                Sign in with Passcode
              </button>

              {loginError && (
                <div className="p-3 bg-red-50 dark:bg-red-950/20 border border-red-200/50 dark:border-red-900/40 rounded-xl">
                  <p className="text-xs text-red-500 font-bold text-center uppercase leading-tight">{loginError}</p>
                </div>
              )}
            </form>
          ...
          <div className="p-3 bg-amber-50/50 dark:bg-emerald-950/20 border border-amber-200 dark:border-emerald-900/50 rounded-xl text-[11px] text-slate-500 dark:text-slate-400 space-y-1.5 leading-normal">
            <span className="font-bold text-amber-700 dark:text-amber-400 uppercase block font-mono">🔧 Credentials Information:</span>
            <p>Authorized Google Email: <code className="font-bold font-mono text-emerald-800 dark:text-emerald-400">ynus335@gmail.com</code></p>
            <div className="pt-1.5 mt-1.5 border-t border-slate-100 dark:border-slate-855 flex justify-between gap-2">
              <div>
                <p className="font-semibold text-emerald-700 dark:text-emerald-400">Bypass Username:</p>
                <code className="font-mono bg-white dark:bg-slate-900 px-1 py-0.5 rounded border border-slate-200 dark:border-slate-700 font-bold">admin</code>
              </div>
              <div>
                <p className="font-semibold text-emerald-700 dark:text-emerald-400">Bypass Passcode:</p>
                <code className="font-mono bg-white dark:bg-slate-900 px-1 py-0.5 rounded border border-slate-200 dark:border-slate-700 font-bold">principal123</code>
              </div>
            </div>
          </div>
        </div>
            ...
            <div className="space-y-10 pb-12">
              <form onSubmit={handleUpdateConfig} className="space-y-8 animate-fade-in">
                ...
                <div className="bg-gradient-to-br from-indigo-500/5 to-emerald-500/5 dark:from-slate-900 dark:to-emerald-950/20 border-2 border-emerald-500/25 dark:border-emerald-500/30 rounded-2xl p-6 shadow-sm space-y-6 animate-fade-in text-xs text-slate-700 dark:text-slate-200">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200/60 dark:border-slate-800/85 pb-4">
                    <div>
                      <h4 className="text-sm font-extrabold text-emerald-700 dark:text-emerald-400 flex items-center gap-2 uppercase tracking-wider">
                        📊 Google Sheets & Database Controls (डाटाबेस एवं गूगल शीट्स)
                      </h4>
                      <p className="text-[11px] text-slate-505 dark:text-slate-400 mt-1 font-medium select-none">
                        नोमान की तालीमी डेटाबेस को सुरक्षित रखने, एक्सेल बैकअप लेने या गूगल शीट्स लिंक करने के लिए नीचे दिए गए टूल्स का उपयोग करें।
                      </p>
                    </div>
                  ...
                  {/* HIGHLY EXPLANATORY GUIDE ON OAUTH BLOCK ISSUE WITH ynus335@gmail.com */}
                  <div className="p-4 bg-amber-50 dark:bg-amber-950/20 border border-amber-250 dark:border-amber-900/40 rounded-xl space-y-2 mb-4 text-[11px] text-amber-850 dark:text-amber-300 leading-relaxed">
                    <div className="flex items-start gap-2">
                      <ShieldAlert className="w-5 h-5 mt-0.5 shrink-0 text-amber-600 dark:text-amber-400" />
                      <div>
                        <strong className="text-xs font-bold block mb-1">💡 Google OAuth Verification Block Information (गूगल ऑथेंटिकेशन ब्लॉक की जानकारी)</strong>
                        <p className="mb-2 font-medium">
                          गूगल सुरक्षा नियमों के अनुसार, चूंकि आपका फायरबेस प्रोजेक्ट नया है, इसलिए यह अभी <strong>"Testing Zone (परीक्षण मोड)"</strong> में है। जब आप नये जीमेल आईडी <strong>ynus335@gmail.com</strong> से लॉग इन करते हैं, तो गूगल आपको "Access Blocked" एरर दिखाता है।
                        </p>
                        <p className="font-bold underline mb-2">
                          इसे पूरी तरह से ठीक करने के लिए आपके पास दो आसान रास्ते हैं:
                        </p>
                        <ul className="list-decimal list-inside space-y-1 ml-1.5 font-medium">
                          <li><strong>रास्ता १ (बिना किसी एरर के तुरंत काम करें):</strong> गूगल पर जाकर खुद एक खाली शीट बनाएं, शेयर सेटिंग्स में <strong>"Anyone with the link can view (कोई भी लिंक वाला व्यक्ति देख सकता है)"</strong> चुनें, फिर नीचे दिए गए इनपुट में उसकी आईडी/यूआरएल पेस्ट करके लिंक कर लें!</li>
                          <li><strong>रास्ता २ (पूरी ऑटो-सिंक ऑन करने के लिए):</strong> अपने Google Cloud Console (APIs & Services {`->`} OAuth consent screen) पर जाएं और <strong>ynus335@gmail.com</strong> को <strong>"Test Users"</strong> की लिस्ट में जोड़ें या उसे <strong>"Publishing Status: In Production"</strong> पर सेट करें।</li>
                        </ul>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <div className="lg:col-span-2 space-y-4">
                      ...
                      {/* SUB-SECTION A: LINK EXISTING GOOGLE SHEET MANUALLY */}
                      <div className="p-4 bg-slate-50 dark:bg-slate-906 border border-slate-205 dark:border-slate-805 rounded-xl space-y-3">
                        <span className="text-[11px] font-extrabold uppercase text-slate-550 dark:text-slate-400 flex items-center gap-1.5">
                          🔗 Manually Link Google Sheet (गूगल शीट मैन्युअल लिंक करें - नो-एरर बायपास)
                        </span>
                        <form onSubmit={handleManualSheetLink} className="flex gap-2 items-center">
                          <input
                            type="text"
                            placeholder="Paste Google Sheet URL or ID here..."
                            value={manualSheetId}
                            onChange={(e) => setManualSheetId(e.target.value)}
                            className="flex-1 p-2 border border-slate-250 dark:border-slate-800 rounded-lg bg-white dark:bg-slate-950 text-slate-800 dark:text-white text-xs font-semibold"
                          />
                          <button
                            type="submit"
                            className="px-3.5 py-2 bg-indigo-600 hover:bg-indigo-750 text-white rounded-lg font-bold text-xs cursor-pointer transition-colors shrink-0"
                          >
                            Link Sheet
                          </button>
                        </form>
                        <p className="text-[10px] text-slate-450 dark:text-slate-550">
                          अगर ऑटो-ऑथ बंद है, तो अपनी गूगल शीट का यूआरएल (जैसे: https://docs.google.com/spreadsheets/d/...) या उसकी आईडी यहाँ पेस्ट करके तुरंत लिंक कर सकते हैं।
                        </p>
                      </div>
                      ...
                      {/* SUB-SECTION B: LOCAL RECOVERY FILE BACKUPS */}
                      <div className="p-4 bg-slate-50 dark:bg-slate-906 border border-slate-205 dark:border-slate-805 rounded-xl space-y-3">
                        <span className="text-[11px] font-extrabold uppercase text-slate-550 dark:text-slate-400 flex items-center gap-1.5">
                          💾 Hard Offline Backup & Recovery (लोकल डेटाबेस सुरक्षित बैकअप फाइल)
                        </span>
                        <div className="flex flex-wrap gap-2 items-center">
                          <button
                            type="button"
                            onClick={handleExportLocalDatabase}
                            className="px-3.5 py-2 bg-slate-200 dark:bg-slate-800 hover:bg-slate-300 text-slate-800 dark:text-white rounded-lg font-bold text-xs cursor-pointer transition-colors flex items-center gap-1.5"
                          >
                            📥 Export Backup File (.json)
                          </button>
                          <label className="px-3.5 py-2 bg-slate-200 dark:bg-slate-800 hover:bg-slate-300 text-slate-800 dark:text-white rounded-lg font-bold text-xs cursor-pointer transition-colors flex items-center gap-1.5">
                        ...
                        <p className="text-[10px] text-slate-450 dark:text-slate-550">
                          अपने पूरे मदरसे के डेटाबेस का एक सेकंड में ऑफलाइन बैकअप डाउनलोड करें। आपके छात्रों, रिजल्ट, गैलरी, नोटिस और दुआओं का पूरा बैकअप इस फाइल में सुरक्षित सुरक्षित हो जायेगा जिसे कभी भी री-अपलोड कर सकते हैं।
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex flex-col justify-center gap-3 bg-slate-50 dark:bg-slate-950/45 p-4 rounded-xl border border-slate-200/60 dark:border-slate-850">
                      <span className="font-extrabold text-[10px] tracking-wider uppercase text-slate-550 block mb-1">
                        🛠️ OAuth Operations Controller
                      </span>
